...
class ClayBackendTools implements tools.spec.BackendTools {
    ...
    public function runSetup(cwd:String, args:Array<String>, target:tools.BuildTarget, variant:String, continueOnFail:Bool = false):Void {
        ...
        var task = new backend.tools.tasks.ClaySetup(target, variant, continueOnFail);
        task.run(cwd, args);
    ...
    public function runBuild(cwd:String, args:Array<String>, target:tools.BuildTarget, variant:String, configIndex:Int = 0):Void {
        ...
        var task = new backend.tools.tasks.ClayBuild(target, variant, configIndex);
        task.run(cwd, args);
            ...
            if (!Files.haveSameLastModified(usedIcon, entry.path)) {
                ...
                if (entry.path.endsWith('.png')) {
                    ...
                    if (entry.padTop != null) {
                        Images.resizeFile(usedIcon, entry.path, entry.width, entry.height, entry.padTop, entry.padRight, entry.padBottom, entry.padLeft);
                    }
                    else {
                        Images.resizeFile(usedIcon, entry.path, entry.width, entry.height);
                    }
