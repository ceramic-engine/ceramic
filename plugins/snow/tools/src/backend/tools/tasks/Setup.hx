...
').ltrim();
        ...
        print('Updated snow project at: $flowPath');
    ...
    function checkFrameworkSetup(forceSetup:Bool = false):Void {
        ...
        if (libs.exists('snow') && !forceSetup) {
            // Snow already available
            return;
        }

        // Install snow (and dependencies)
        //
        print('Install snow\u2026');
        ...
        // Check that snow is now available
        //
        ...
        if (!libs.exists('snow')) {
            // Snow still not available?
            fail('Failed to install snow or some of its dependency. Check log.');
        }
