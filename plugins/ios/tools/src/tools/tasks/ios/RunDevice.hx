...
Optional, to use manual signing instead of automatic:
    ...
    export CERAMIC_IOS_PROVISIONING_PROFILE="Your Provisioning Profile Name"');
        ...
        // Compile the native code now so the app xcframework is fresh before
        // xcodebuild plans the build: XCBuild snapshots xcframework slices at
        // planning time, so a slice regenerated by the in-build script phase
        // would only be picked up by the NEXT build. The in-build phase then
        // only has to verify everything is up to date.
        //
        runTask('ios compile', ['--archs', 'arm64']);
...
Developer Mode is enabled (Settings > Privacy & Security > Developer Mode).');
    ...
    function resolveDevice(outTargetPath:String, requestedDevice:String):{ identifier:String, udid:String, name:String } {
        ...
        // A paired device listed as 'disconnected' is still usable: devicectl
        // establishes the tunnel on demand. Prefer devices with an active
        // tunnel, but don't require one.
        var tunnelStatePriority = ['connected', 'disconnected', 'unavailable'];

        var devices:Array<{ identifier:String, udid:String, name:String, priority:Int }> = [];
        ...
        for (jsonDevice in jsonDevices) {
            ...
            var pairingState:String = jsonDevice.connectionProperties != null ? jsonDevice.connectionProperties.pairingState : null;
            ...
            if (platform == 'iOS' && pairingState == 'paired') {
                var priority = tunnelStatePriority.indexOf(tunnelState);
                devices.push({
                    ...
                    priority: priority >= 0 ? priority : tunnelStatePriority.length
                });
            }
        }
