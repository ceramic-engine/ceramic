...
class HeadlessBackendTools implements tools.spec.BackendTools {
    ...
    public function runSetup(cwd:String, args:Array<String>, target:tools.BuildTarget, variant:String, continueOnFail:Bool = false):Void {
        ...
        var task = new backend.tools.tasks.HeadlessSetup(target, variant, continueOnFail);
        task.run(cwd, args);
    ...
    public function runBuild(cwd:String, args:Array<String>, target:tools.BuildTarget, variant:String, configIndex:Int = 0):Void {
        ...
        var task = new backend.tools.tasks.HeadlessBuild(target, variant, configIndex);
        task.run(cwd, args);
