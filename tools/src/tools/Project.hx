...
class ProjectLoader {
    ...
    public static function loadAppConfig(
        ...
        try {
            ...
            // `gen/` is generated by the build itself (asset name classes, plugin
            // templates, bind hooks), so it may not exist yet when the project is
            // loaded: always declare the class path instead of depending on what is
            // currently on disk, otherwise a first build without `gen/` writes an hxml
            // that lacks it and fails, while the second one succeeds.
            var paths:Array<String> = app.paths;
            if (paths.indexOf('gen') == -1) {
                paths.push('gen');
            }
